<template>
  ...
  <div class="modalWrapper">
    ...
    <div v-if="QnaDetail != undefined">
      ...
      <label>파일명: {{fileName!=''? fileName: '' }}</label>
      <div>
        <label>
          비밀번호입력 :
 ...
 <label v-if="type!='M' && QnaDetail.ans_conten!=''">
          <span class="ans_content">질문에대한 관리자님의 답변입니다. 답변입니다. :</span>
         <input type="text" v-model="QnaDetail.ans_content" :disabled="type!='M'">
        </label>
      ...
      <div class="updateDeleteArea">
        <div class="myArea"
          v-if="
            userInfo.user.loginId == QnaDetail.author || userInfo.user.userType === 'M'
          "
        >
          ...
          <div class="delete" @click="handlerDeleteBtn(QnaDetail.qnaIdx)">삭제</div>
        </div>
...
const { mutate:myQnaUpdate}=useQnaMyUpdateMutation(fileData, userInfo, QnaDetail);
