  ...
  <template v-for="(item, key) in careerList?.payload" :key="key">
    <div class="career_table">
      ...
      <div class="garo_wrapper_r" style="grid-area: button">
        <CommonButton @click="handlerDeleteCareerBtn({ resIdx: props.resume.resIdx, crrIdx: item.crrIdx })" v-if="props.isEditor">삭제</CommonButton>
      </div>
    </div>
  </template>
  <button class="add_btn" @click="isAddCareer = !isAddCareer" v-if="props.isEditor">+ 추가</button>
  <div>
    <div class="career_table" v-if="isAddCareer && props.isEditor">
      <div class="garo_wrapper_lr" style="grid-area: startDate">
        <label class="garo_wrapper_lr_l">입사일:</label>
